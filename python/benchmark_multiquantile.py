...
def run_experiment(params, device_info):
    ...
    trial, scenario_idx, nidx, hidx, kidx, X_test, y_quantiles, \
    ...
    model_id = f"{functions[0].__class__.__name__}_{model_shape}_N{sample_sizes[nidx]}_{kernels[kidx]}_{'' if residual else 'no'}res"
    if kidx == 0:
    ...
    # Save the models for trial 1, sample size 10000
    if trial == 0 and nidx == 2:
        torch.save(model, f'data/model_multiquantile/{model_id}')
    ...
    mse_value = ((y_quantiles - preds)**2).mean(axis=0)
    mae_value = np.abs(y_quantiles - preds).mean(axis=0)
    
    # if trial % 5 == 0:
    #     print(f"Completed {trial+1} trials")

    return (trial, scenario_idx, hidx, kidx, nidx, 
...
def run_benchmarks(demo=True):
    ...
    print(f"Quantile levels = {quantiles}, multiple non-crossing quantiles")
        ...
        for scenario_idx, func in enumerate(functions):
            ...
            for nidx, N_train in enumerate(sample_sizes):
                ...
                for kidx, kernel in enumerate(kernels):
                    if kidx == 0:
                        model = QuantileNetwork(quantiles=quantiles,shape=model_shape,residual=residual)
                        params = (trial, scenario_idx, nidx, 0, kidx, X_test, y_quantiles, 
                                manual_grad, stop, X_train, y_train, model)
                        all_params.append(params)
                    else:
                        for hidx, h in enumerate(bandwidths):
                            model = ConquerNetwork(quantiles=quantiles,kernel=kernel,bandwidth=h,shape=model_shape,residual=residual)
                            params = (trial, scenario_idx, nidx, hidx, kidx, X_test, y_quantiles, 
                                manual_grad, stop, X_train, y_train, model)
                            all_params.append(params)
    ...
    for result in results:
        trial, scenario_idx, hidx, kidx, nidx, mse_value, mae_value, training_time, train_loss, val_loss = result
        
        if kidx == 0:
            mse_results[trial, scenario_idx, :, kidx, nidx, :] = np.tile(mse_value, (len(bandwidths),1))
            mae_results[trial, scenario_idx, :, kidx, nidx, :] = np.tile(mae_value, (len(bandwidths),1))
            time_results[trial, scenario_idx, :, kidx, nidx, :] = np.tile(training_time,(len(bandwidths),1))
            train_losses[trial, scenario_idx, :, kidx, nidx, :, :] = np.tile(train_loss.T,(len(bandwidths),1,1))
            val_losses[trial, scenario_idx, :, kidx, nidx, :, :] = np.tile(val_loss.T,(len(bandwidths),1,1))
        else:
            mse_results[trial, scenario_idx, hidx, kidx, nidx, :] = mse_value
            mae_results[trial, scenario_idx, hidx, kidx, nidx, :] = mae_value
            time_results[trial, scenario_idx, hidx, kidx, nidx, :] = training_time
            train_losses[trial, scenario_idx, hidx, kidx, nidx, :, :] = train_loss.T
            val_losses[trial, scenario_idx, hidx, kidx, nidx, :, :] = val_loss.T
    ...
    result_path = f'data/model_multiquantile/{model_id}.npz'
