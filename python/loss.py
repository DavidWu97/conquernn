...
class QuantileLoss:
    ...
    def gaussian(self, yhat, y, requires_grad=True):
        ...
        out1 = (
            ...
            * h
            * (
                self.sqrt_2_over_pi.to(z.device) * exp_term
                + normalized_z * (2 * pnorm_val - 1)
            )
            + (q[None] - 0.5) * z
        )
    ...
    def uniform(self, yhat, y, requires_grad=True):
        ...
        out1_term = torch.where(
            abs_normalized_z <= 1, 0.5 * normalized_z**2 + 0.5, abs_normalized_z
        )
        out1 = 0.5 * h * out1_term + (q[None] - 0.5) * z

        out2_term = torch.where(abs_normalized_z <= 1, normalized_z, sign_normalized_z)
...
class PinballLoss(nn.Module):
    """Pinball loss for one quantile level."""
...
class MultiQuantilePinballLoss(nn.Module):
    """Average pinball loss for several quantile levels."""
