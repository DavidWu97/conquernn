"""Simulation data-generating processes used in the paper."""
...
class Scenario1:
    ...
    def noiseless(self, X):
        return np.cos(2 * np.pi * X[:, 0] ** 2) + np.sin(
            np.sqrt(X[:, 0] ** 2 + 2 * X[:, 1]) + 2
        )
...
class Scenario2:
    ...
    def sample(self, X):
        return self.noiseless(X) + np.sqrt(X.dot(self.beta)) * np.random.standard_t(
            3, size=X.shape[0]
        )
...
class Scenario3:
    ...
    def g1(self, X):
        return np.array(
            [
                X[:, 0] + 3 * X[:, 1],
                np.cos(2 * np.pi * (X[:, 2] + X[:, 3])),
                X[:, 1] + np.sqrt(X[:, 2]) + 2 * X[:, 4],
            ]
        ).T
    ...
    def g2(self, X):
        ...
        return np.where(
            condition,
            X[:, 0] + np.sqrt(X[:, 1] ** 2 + X[:, 2]),
            np.sqrt(X[:, 0] + X[:, 1]) + 0.5 * X[:, 2],
        )
